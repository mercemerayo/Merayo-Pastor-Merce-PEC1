...
## Carga de los datos desde `www.metabolomicsworkbench.org`

```{r cargas_datos, warning=FALSE, message=FALSE}
url <- "https://www.metabolomicsworkbench.org/data/showfile_t.php?RA=185.110.33.198&DF=MSdata_ST003471_1.txt"
# Cargar datos del experimento
data_ST003471 <- read.delim(url, sep = "\t", header = TRUE, skip = 1)  

head(data_ST003471[,1:5])
```

En los datos recuperados hay información sobre los metabolidos considerados en el estudio. Por otro lado, hay identificadores de las muestras que incluyen información sobre el tratamiento en el propio identificador. Adicionalmente, hay información sobre el origen anatómico de la muestras, en este caso del hipocampo, tipo de tratamiento y el y el tiempo *Zeitgeber* (`Zeitgeber_time`). Este último, en estudios circadianos, hace referencia al tiempo en función de un sincronizador ambiental, normalmente el ciclo luz/oscuridad, donde "ZT" se refiere a la hora en relación con el comienzo del periodo de luz (ZT11) [2].
...
Se construye un objeto `SummarizedExperiment` que integra los datos de expresión, información de las muestras y detalles de los metabolitos.
...
se <- SummarizedExperiment(
  ...
  rowData = datos_filas,
  metadata = list(info_muestras = info_muestras)
)
